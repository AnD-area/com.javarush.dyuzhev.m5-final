...
public interface ActivityRepository extends BaseRepository<Activity> {
    ...
    @Query("SELECT a FROM Activity a WHERE a.taskId =:taskId AND a.statusCode =:statusCode ORDER BY a.updated ASC")
    List<Activity> findFirstByTaskIdAndStatusCodeOrderByUpdatedAsc(long taskId, String statusCode);
}
